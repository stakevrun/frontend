...
export function trimValue(
  ...
  if (amountEthish.length <= maxLength) {
    return `${ethers.commify(amountEthish)}`;
  }
  return `${ethers.commify(amountEthish.substring(0, maxLength))}…`;
}
...
export const distributeBalanceInterface = new ethers.Interface([
  {
    type: "function",
...
export const distributeInterface = new ethers.Interface([
  {
    type: "function",
    name: "distribute",
    inputs: [],
...
export const delegateUpgradeInterface = new ethers.Interface([
  {
    type: "function",
