...
const RPLBlock: NextPage = () => {
    ...
    const handleCheckStakeRPL = async (add: string) => {
      ...
      if (typeof (window as any).ethereum !== "undefined") {
        ...
        try {
          ...
          const possibleNewMinpools = LEB8sPossible - ethers.parseEther(activeMinipools.toString());
  ...
  return (
    <div className="flex flex-col  h-auto gap-2 text-center items-center justify-center rounded-lg border shadow border-b-4 border-black-100 px-2 py-2 ">
    ...
    <p className="my-4 w-[80%] text-gray-500 sm:text-l">
      ...
      one active Minipool and are able to create <span className={`text-green-500 font-bold`} style={newMinipools < 1 ? { color: "rgb(34 197 94)" } : { color: "red" }}> <RollingNumber n={Math.floor(Number(ethers.formatEther(newMinipools)))} /></span> new LEB8s (Minipools)
