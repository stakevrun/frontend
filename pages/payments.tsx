...
import {
  getPaymentsData,
  getPaymentsEntryData,
} from "../globalredux/Features/payments/paymentSlice";
import {
  getChargesData,
  getChargesEntryData,
} from "../globalredux/Features/charges/chargesSlice";
...
const Payments: NextPage = () => {
  ...
  const reduxChargesEntries = useSelector(
    (state: RootState) => state.chargesData.entryData
  );
  const reduxPaymentsEntries = useSelector(
    (state: RootState) => state.paymentsData.entryData
  );
      ...
      try {
        ...
        if (e.reason) {
          ...
          setPaymentErrorMessage(
            "Error: check you have input a valid ETH value."
          );
        }
  ...
  useEffect(() => {
    ...
    const newArray = sortInReverseChronologicalOrder([
      ...reduxChargesEntries,
      ...reduxPaymentsEntries,
    ]);
  ...
  function sortInReverseChronologicalOrder(
    array: Array<edittedObject>
  ): Array<edittedObject> {
    return array
      .slice()
  ...
  const [showFormMakePaymentEffect, setShowFormMakePaymentEffect] =
    useState(false);
            ...
            <>
              <div className="w-full min-h-[92vh] h-auto flex flex-col items-center justify-center gap-[8vh] ">
                ...
                <div className=" w-auto p-5 border  rounded-lg flex justify-start items-center shadow-xl">
                  ...
                  <div className="flex flex-col items-center justify-center">
                      ...
                      <span
                        ...
                        style={
                          reduxPayments - reduxCharges > 0
                            ? { color: reduxDarkMode ? "#fff" : "green" }
                            : reduxPayments - reduxCharges === 0
                            ? { color: reduxDarkMode ? "#fff" : "black" }
                            : { color: "red" }
                        }
                      >
                    ...
                    {reduxPayments - reduxCharges >= 0 ? (
                      <span className="block text-md lg:text-lg text-gray-500 ">
                        Vrün Balance
                      </span>
                    ) : (
                      <span className="block text-md lg:text-lg text-gray-500 ">
                ...
                <div className="flex items-center justiify-center w-auto h-full flex-col shadow-xl border rounded-lg gap-2  px-6 py-6 pt-[45px] text-center">
                  <div className=" w-[85%] flex items-center flex-col justify-center gap-2">
                    <h2 className=" text-xl lg:text-2xl  self-start font-bold mb-2">
                      Add ETH Credit:
                    </h2>
              ...
              <div className="w-full min-h-[92vh] h-auto flex flex-col items-center justify-center gap-8 py-[8vh]">
                {totalData.length > 0 ? (
                  <div
                    id="accountTable"
                    className="w-[90%] sm:w-[80%] lg:w-auto overflow-x-scroll shadow-xl border rounded-lg"
                  >
                          ...
                          <tr key={index}>
                            <td className="px-4 py-5 w-[200px]">
                              <span className="text-center text-black-500 text-md lg:text-xl">
                                <p>
                                  {data.date !== "defaultState" && data.date}
                                </p>
                              </span>
                            </td>
                              ...
                              <span className="text-center text-grey-700 font-bold text-sm lg:text-lg">
                                {data.pubkey === "payment" ? (
                                  <p className="text-center">
                                    Vrün Account Deposit
                                  </p>
                                ) : (
                                  <p className="text-center">
                                    Daily Validator Charge
                                  </p>
                                )}
                              </span>
                            </td>
                  ...
                  <div className="h-[100vh] w-full flex items-center gap-2 justify-center flex-col">
                    <h3 className="text-center max-w-[90%]">
                      Please wait while we retrieve your Vrün statement...{" "}
                    </h3>
