...
const CreateValidator: NextPage = () => {
    ...
    if (!isInitialRender) {
      const getMinipoolTruth = async () => {
        ...
        try {
          ...
          const NodeManagerAddress = await storageContract[
            "getAddress(bytes32)"
          ](ethers.id("contract.addressrocketNodeManager"));
          ...
          const bool =
            await rocketNodeManager.getSmoothingPoolRegistrationState(address);
  ...
  const handleCheckRPL = async (add: string) => {
    if (typeof (window as any).ethereum !== "undefined") {
      try {
        ...
        const tokenAddress = await storageContract["getAddress(bytes32)"](
          ethers.id("contract.addressrocketTokenRPL")
        );
  ...
  const handleStakeButtonClick = async () => {
    try {
      ...
      if (nodeAddress) {
        ...
        setStakingMessage(
          "Approval sucessful, now initiating Stake transaction..."
        );
        ...
        for (const [minAddress, pubkey] of attachedPubkeyArray) {
          if (minAddress === "Null minipool") {
            ...
            const timeRemaining: number =
              numScrub - (Date.now() - numStatusTime);
            ...
            let newBeaconStatusObject = await getBeaconchainStatusObject(
              pubkey
            );
            ...
            const newFeeRecipient = await getFeeRecipient(
              pubkey,
              smoothingBool
            );
            ...
            seperateMinipoolObjects.push({
              ...
              beaconStatus:
                typeof beaconStatus === "string" ? beaconStatus : "",
              activationEpoch:
    ...
    if (address !== undefined && grafittiInput !== "") {
      try {
        ...
        const TermsOfServiceTypes = {
          AcceptTermsOfService: [{ name: "declaration", type: "string" }],
        };

        // Get acceptance sheet (currently not working)

        await fetch(
          `https://api.vrün.com/${currentChain}/${address}/acceptance`,
          {
            method: "GET",
          ...
          .then(async (response) => {
            ...
            console.log(
              "Result of Acceptance GET" + Object.entries(jsonString)
            );
          ...
          .catch(async (error) => {
            const requiredDeclaration =
              "I accept the terms of service specified at https://vrün.com/terms (with version identifier 20240229).";

            const TermsOfServiceValue = {
              declaration: requiredDeclaration,
            };

            let TermsOfServiceSignature = await signer.signTypedData(
              EIP712Domain,
              TermsOfServiceTypes,
              TermsOfServiceValue
            );
            ...
            try {
              const response: any = await fetch(
                `https://api.vrün.com/${currentChain}/${address}`,
                {
                  method: "PUT",
                  headers: {
                    "Content-Type": "application/json",
        ...
        const baseAddress = await storageContract["getAddress(bytes32)"](
          ethers.id("contract.addressrocketMinipoolBase")
        );
        const initHash = ethers.keccak256(
          `0x3d602d80600a3d3981f3363d3d373d3d3d363d73${baseAddress.slice(
            2
          )}5af43d82803e903d91602b57fd5bf3`
        );

        const MinipoolFactoryAddress = await storageContract[
          "getAddress(bytes32)"
        ](ethers.id("contract.addressrocketMinipoolFactory"));
        ...
        const keccakedSalt = ethers.keccak256(
          ethers.concat([address, defaultSalt])
        );

        //const salt = await browserProvider.getTransactionCount(address)

        const newMinipoolAddress = ethers.keccak256(
          ethers.concat([
            "0xff",
            MinipoolFactoryAddress,
            keccakedSalt,
        ...
        const value = {
          ...
          amountGwei: (
            parseEther("1") / ethers.parseUnits("1", "gwei")
          ).toString(),
        ...
        try {
          const response: any = await fetch(
            `https://api.vrün.com/${currentChain}/${address}/${currentIndex}`,
            {
              method: "POST",
              headers: {
                "Content-Type": "application/json",
          ...
          ({ depositDataRoot, signature: depositSignature } =
            resJSON[firstPubkey]);
        ...
        await fetch(
          `https://api.vrün.com/${currentChain}/${address}/pubkey/${currentIndex}`,
          {
            method: "GET",
        ...
        const NodeDepositAddress = await storageContract["getAddress(bytes32)"](
          ethers.id("contract.addressrocketNodeDeposit")
        );
        ...
        if (selectedCont === "16 ETH") {
          result = await depositContract.deposit(
            ethers.parseEther("16"),
            ethers.parseEther("0.14"),
            generatedPubKey,
            depositSignature,
            depositDataRoot,
            ethers.hexlify(defaultSalt),
            fixedNewMinipoolAddress,
  ...
  const handleOptSmoothingPool = async () => {
    ...
    if (typeof (window as any).ethereum !== "undefined") {
      try {
        ...
        if (input.reason) {
          setErrorBoxTest2(
            input.info.error.message.toString() +
              " PLEASE NOTE: there is a 'cooldown' period after every Smoothing Pool change and you may not be able to toggle it for a few days... "
          );
        } else if (input.error) {
          setErrorBoxTest2(
            input.error["message"].toString() +
              " PLEASE NOTE: there is a 'cooldown' period after every Smoothing Pool change and you may not be able to toggle it for a few days... "
          );
        } else {
  ...
  const [showFormEffectUnstakeRPL, setShowFormEffectUnstakeRPL] =
    useState(false);
  ...
  return (
    <div
      ...
      className="flex w-full flex-col items-center bg-white justify-center "
    >
      <Head>
              ...
              {checkTruth ? (
                <>
                  {(Number(formatEther(newMinipools)) < 1 ||
                    Number(formatEther(newMinipools)) -
                      Math.floor(Number(displayActiveMinipools)) -
                      prelaunched <
                      1 ||
                    Math.floor(Number(formatEther(newMinipools))) <=
                      Math.floor(Number(displayActiveMinipools))) && (
                    <div className="flex flex-col  gap-2 w-[500px] rounded-xl border border-black-100 px-4 py-[5vh] text-center shadow-xl items-center justify-center flex items-center p-8 shadow rounded-lg border">
                        ...
                        <p className="my-4 w-[80%] text-gray-500 sm:text-l">
                          ...
                          <span className="text-yellow-500 font-bold">
                            ...
                            {roundToTwoDecimalPlaces(
                              ethers.formatEther(RPL)
                            )}{" "}
                          </span>
                          unstaked RPL in your Wallet and
                          <span
                            style={
                              Number(ethers.formatEther(stakeRPL)) >= 1
                                ? { color: "rgb(34 197 94)" }
                                : { color: "red" }
                            }
                            className="font-bold"
                          ...
                          <span
                            ...
                            style={
                              displayActiveMinipools >= 1
                                ? { color: "rgb(34 197 94)" }
                                : { color: "red" }
                            }
                          >
                            {" "}
                            {Number(displayActiveMinipools)}{" "}
                          </span>
                          active Minipool(s) with a maximum total of{" "}
                          <span
                            className={`text-green-500 font-bold`}
                            style={
                              Math.floor(
                                Number(ethers.formatEther(newMinipools))
                              ) < 1
                                ? { color: "red" }
                                : { color: "rgb(34 197 94)" }
                            }
                      ...
                      <div className="flex flex-col gap-2 items-center justify-center">
                        <input
                          ...
                          className="self-center  w-[80%] bg-gray-100 text-lg py-4 px-3 rounded-xl shadow-lg border border-black-200 text-gray-500"
                          style={
                            stakeButtonBool
                              ? { display: "block" }
                              : { display: "none" }
                          ...
                          type="text"
                          onChange={handleRPLInputChange}
                        />
                        <button
                          onClick={() => {
                            setMaxUnstakedRPL();
                          }}
                          className="bg-black mt-2 text-sm hover:bg-gray-700 text-white font-bold py-2 px-4 rounded-md"
                        >
                          MAX
                        </button>
                      </div>
                  ...
                  <div
                    ...
                    className="flex flex-col w-[500px] gap-2 rounded-xl border border-black-100 px-4 py-[5vh] text-center shadow-xl items-center justify-center flex items-center p-8 shadow rounded-lg border"
                  >
                    ...
                    <input
                      value={grafittiInput}
                      placeholder="Grafitti"
                      className="mt-4 mb-2 border border-black-200 "
                      type="text"
                      onChange={handleGrafittiInput}
                    />

                    <div className="w-[80%] mt-2">
                      <label className="text-gray-500 mb-3 sm:text-l">
                        {" "}
                        Please select ETH Deposit Value:
                      </label>
                ...
                <div className="w-full h-[auto] gap-2  flex flex-col items-center justify-center p-8 px-[6vh]">
                  <h3 className="text-center w-[90%]">
                    Preparing Create Validator environment...
                  </h3>
              ...
              >
                <div className="flex relative w-full h-full items-center justify-center flex-col rounded-lg gap-2 bg-gray-100 px-6 py-6 pt-[45px] text-center">
                  ...
                  <h2 className="text-2xl font-bold text-gray-900 sm:text-2xl">
                    Would you like to Opt into the Smoothing Pool?
                  </h2>

                  <p className="my-4 w-[90%] text-gray-500 sm:text-lg">
                    Tick yes and confirm your changes, or exit this modal to
                  ...
                  <div className="w-full flex gap-3 items-center justify-center">
                    ...
                    <button
                      ...
                      className="bg-yellow-500 mt-2  hover:bg-yellow-700 text-white font-bold py-2 px-4 rounded-md"
                    >
                      Close Modal
                    </button>
                  </div>
                        ...
                        <div className="flex items-start justify-between gap-6 w-full">
                          <div className="flex items-center justify-start gap-4">
                            ...
                            <p className="text-left">
                              Checking Terms and Conditions Acceptance
                            </p>
                          </div>
