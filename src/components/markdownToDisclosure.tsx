    ...
    <div className="w-full p-2 mx-auto rounded-2xl">
      {groups.map(({ header, content }, index) => (
        <Disclosure key={index} as="div" className="mt-2">
          {({ open }) => (
            <>
              <DisclosureButton className="w-full px-4 py-2 text-md font-medium text-zinc-900 bg-zinc-100 rounded-lg hover:bg-zinc-200 focus:outline-none focus-visible:ring-1 focus-visible:ring-zinc-400 focus-visible:ring-opacity-75">
                <span>
                  {React.isValidElement(header)
                    ? header.props.children
                    : header}
                {/* {open ? <CiCircleMinus /> : <CiCirclePlus />} */}
                </span>
              </DisclosureButton>
